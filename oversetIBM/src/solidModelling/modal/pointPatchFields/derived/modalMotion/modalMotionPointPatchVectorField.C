    ...
    nModes_(0)
{
    ...
    {
        FatalIOErrorIn("modalMotionPointPatchVectorField::modalMotionPointPatchVectorField" "(...)", dict)
    ...
    if (dict.found("modeShapes"))
    {
        ...
        {
            FatalIOErrorIn("modalMotionPointPatchVectorField::modalMotionPointPatchVectorField" "(...)", dict)
                << "Incorrect number of mode shapes specified. All linear and quadratic mode shapes must be included."
                << exit(FatalIOError);
        }
        forAll(modeShapes_, modeI)
        {
            ...
            {
                FatalIOErrorIn("modalMotionPointPatchVectorField::modalMotionPointPatchVectorField" "(...)", dict)
                    << "Size of mode shape " << modeI << " is " << modeShapes_[modeI].size()
...
:
    ...
    modalFunctionObjectName_(ptf.modalFunctionObjectName_),
    modeShapes_(ptf.modeShapes_), // Doesn't actually copy the contents; done below
    nModes_(ptf.nModes_)
{
    forAll(ptf.modeShapes_, i)
    {
...
)
{
    ...
    {
        FatalErrorIn("void modalMotionPointPatchVectorField::rmap(...)") << "Mode shapes have inconsistent size." << abort(FatalError);
    }
...
void modalMotionPointPatchVectorField::updateCoeffs()
{
    ...
    label funObjI = mesh.time().functionObjects().findObjectID(modalFunctionObjectName_);
    if (funObjI == -1)
    {
        FatalErrorIn("modalMotionPointPatchVectorField::updateCoeffs()")
            << "No function object '" << modalFunctionObjectName_ << "' was found." << exit(FatalError);
    }
    else
    {
        try
        {
            const_cast<modalSolver&>(
                dynamic_cast<const modalSolver&>(
                    mesh.time().functionObjects()[funObjI])).getPatchDispl(mesh.name(), patch().index(), modeShapes_, displ);
        }
        catch(const std::bad_cast&)
        {
            FatalErrorIn("modalMotionPointPatchVectorField::updateCoeffs()")
                << "Function object '" << modalFunctionObjectName_ << "' must be of type '"
                << modalSolver::typeName_() << "'." << exit(FatalError);
        }
    }
...
void modalMotionPointPatchVectorField::write(Ostream& os) const
{
    ...
    os.writeKeyword("modalFunctionObject") << modalFunctionObjectName_ << token::END_STATEMENT << nl;
