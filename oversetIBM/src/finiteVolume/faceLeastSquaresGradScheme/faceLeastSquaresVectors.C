...
void Foam::faceLeastSquaresVectors::makeLeastSquaresVectors() const
{
    ...
    forAll(owner, facei)
    {
        ...
        //~ Division by weightSum here and below to prevent numerical issues with inversion of tiny tensor
        dd[own] += weight[facei]/weightSum[own]*Ce[facei]*(Ce[facei]-meanPoints[own]);
        dd[nei] += weight[facei]/weightSum[nei]*Ce[facei]*(Ce[facei]-meanPoints[nei]);
    }
    ...
    forAll(blsP, patchi)
    {
        ...
        forAll(pw, patchFacei)
        {
            dd[faceCells[patchFacei]] +=
                pWeight[patchFacei]/weightSum[faceCells[patchFacei]]*pCe[patchFacei]*(pCe[patchFacei]-meanPoints[faceCells[patchFacei]]);
        }
    }
    ...
    forAll(owner, facei)
    {
        ...
        lsP[facei] = weight[facei]/weightSum[own]*(invDd[own] & Ce[facei]);
        lsN[facei] = weight[facei]/weightSum[nei]*(invDd[nei] & Ce[facei]);
    }
    ...
    forAll(blsP, patchi)
    {
        ...
        {
            patchLsP[patchFacei] =
               pWeight[patchFacei]/weightSum[faceCells[patchFacei]]*(invDd[faceCells[patchFacei]] & Ce.boundaryField()[patchi][patchFacei]);
        }
    }
