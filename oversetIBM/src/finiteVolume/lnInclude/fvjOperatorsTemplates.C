...
)
{
    ...
    tmp<GeometricField<scalar, fvsPatchField, surfaceMesh> > sf2 = sf*mesh.magSf()*mesh.deltaCoeffs();
    ...
    forAll(owner, facei)
    {
        ...
        low[facei] = vf[own]*sf2()[facei]; //TODO: Don't assign to lower, keep symmetric?
    }
    mx.negSumDiag();
    mx.interfacesUpper().resize(mesh.boundary().size());
    mx.interfacesLower().resize(mesh.boundary().size());
...
)
{
    ...
    forAll(upp, facei)
    {
        upp[facei] = sf[facei]*mesh.magSf()[facei];
        low[facei] = sf[facei]*mesh.magSf()[facei];
    }
    mx.negSumDiag();
    mx.interfacesUpper().resize(mesh.boundary().size());
    mx.interfacesLower().resize(mesh.boundary().size());
    forAll(mesh.boundary(), patchi)
    {
        mx.interfacesUpper().set(patchi, sf.boundaryField()[patchi]*mesh.magSf().boundaryField()[patchi]);
        mx.interfacesLower().set(patchi, sf.boundaryField()[patchi]*mesh.magSf().boundaryField()[patchi]);
