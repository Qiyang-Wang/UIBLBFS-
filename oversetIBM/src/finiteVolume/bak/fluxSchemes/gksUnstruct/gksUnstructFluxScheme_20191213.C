...
#include "gksUnstructFluxScheme.H"
...
defineTypeNameAndDebug(gksUnstructFluxScheme, 0);
addToRunTimeSelectionTable(fluxScheme, gksUnstructFluxScheme, dictionary);
...
gksUnstructFluxScheme::gksUnstructFluxScheme
(
...
:
    ...
    (
        ...
        mesh_,
        dimensionedScalar( "slen", dimensionSet(0,1,0,0,0,0,0), 0.0025 )
    )
{
    calcStreamingDistant(slen_);
}
...
void Foam::gksUnstructFluxScheme::calcFlux(surfaceScalarField& phi, surfaceVectorField& phiUp, surfaceScalarField& phiEp, surfaceVectorField& Up)
{
...
//    scalar Cspeed(Foam::sqrt(3.0)*400);
    scalar Cspeed(Foam::sqrt(3.0*287*288.15));
//    scalar Cspeed(Foam::sqrt(3.0*287*0.3));
...
//		    scalar nuFace = 1.5e-5;//nuEff.boundaryField()[iPatch][ii];
...
void Foam::gksUnstructFluxScheme::viscousFlux
(
    vector n, 
    scalar rho, 
    scalar P,
...
void Foam::gksUnstructFluxScheme::icoGKSFlux
(
...
// Step 2: compute Roe averged quantities for face:
    ...
    scalar dt  = 0.4*streamingLength/( max(max(mag(wM2),mag(wM3)),mag(wM4)) +Cspeed );
...
void Foam::gksUnstructFluxScheme::calcStreamingDistant(surfaceScalarField& slen)
{
...
//    Info << "faces: " << mesh_.faces().size() << endl;
//    Info << "Sf: " << mesh_.Sf().size() << endl;
//    cin.get();
...
/*	//const  vector & x1= mesh.Sf()[iface];
        forAll(mesh_.faces()[iface],pt)
        {
//	    vector x2= mesh_.points()[mesh_.faces()[iface][pt]] - surfacecenter[iface];
	    vector x2(0,0,0);
	    if (pt < (mesh_.faces()[iface].size()-1))
            {
	        x2= (mesh_.points()[mesh_.faces()[iface][pt]] + mesh_.points()[mesh_.faces()[iface][pt+1]])/2- surfacecenter[iface];
            } else {
	        x2= (mesh_.points()[mesh_.faces()[iface][pt]] + mesh_.points()[mesh_.faces()[iface][0]])/2- surfacecenter[iface];
            }
...
*/
	    ...
	    if (pt < (mesh_.faces()[iface].size()-1))
            {
		h = triangleHeight(face_center,mesh_.points()[mesh_.faces()[iface][pt]],mesh_.points()[mesh_.faces()[iface][pt+1]]);
            } else {
		h = triangleHeight(face_center,mesh_.points()[mesh_.faces()[iface][pt]],mesh_.points()[mesh_.faces()[iface][0]]);
            }
	    length = Foam::min(length, h);
        }
        ...
        slen[iface]= Foam::min(length , Foam::min(a,b));
	...
	if (mesh_.boundaryMesh()[iPatch].coupled())
	{
		forAll(mesh_.boundaryMesh()[iPatch], fi)
		{
		    ...
		    //const  vector & x1= mesh.Sf()[iface];
/*                    forAll(mesh_.faces()[faceLabel],pt)
                    {
//	                vector x2= mesh_.points()[mesh_.faces()[faceLabel][pt]] - face_center;
	                vector x2(0,0,0);
	                if (pt < (mesh_.faces()[faceLabel].size()-1))
                        {
	                    x2= (mesh_.points()[mesh_.faces()[faceLabel][pt]]+mesh_.points()[mesh_.faces()[faceLabel][pt+1]])/2 - face_center;
                        } else {
	                    x2= (mesh_.points()[mesh_.faces()[faceLabel][pt]]+mesh_.points()[mesh_.faces()[faceLabel][0]])/2 - face_center;
                        }
	                length = Foam::min(length, mag(x2));
                    }
*/
			...
			scalar h;
	                if (pt < (mesh_.faces()[faceLabel].size()-1))
                        {
                            h = triangleHeight(face_center,mesh_.points()[mesh_.faces()[faceLabel][pt]],mesh_.points()[mesh_.faces()[faceLabel][pt+1]]);
                        } else {
                            h = triangleHeight(face_center,mesh_.points()[mesh_.faces()[faceLabel][pt]],mesh_.points()[mesh_.faces()[faceLabel][0]]);
                        }
	                length = Foam::min(length, h);
                    }
		    //Get cell center
		    ...
		    a= mag(leftCenter-face_center);
		    b= mag(rightCenter-face_center);
		    slen.boundaryFieldRef()[iPatch][fi]= Foam::min(length , Foam::min(a,b));
	        }
	 }
    }
...
scalar Foam::gksUnstructFluxScheme::triangleHeight
(
    vector c,
    vector f1,
    vector f2
)
{
    ...
    vector Area = arm1^arm2;
    
    return mag(Area)/2/mag(arm1);
}
...
/*
    label iface = 642674;
    Info << "iface: " << iface << endl;
    Info << "slen[iface]: " << slen[iface] << endl;
    vector x1(0,0,0);
    forAll(mesh_.faces()[iface],pt)
    {
        Info << "location: " << (mesh_.points()[mesh_.faces()[iface][pt]]) << endl;
        x1 += mesh_.points()[mesh_.faces()[iface][pt]];
    }
    x1 /= mesh_.faces()[iface].size();
    Info << "faceCenter: " << surfacecenter[iface] << endl;
    Info << "average: " << x1 << endl;
    Info << "distance: " << (x1-surfacecenter[iface]) << endl;
	...
	const labelList& edgenumberfacei = faceEdges[iface];
		...
		length = Foam::min(length, mag(x2));
                ...
                if (iface == 29587198) {
                    Info << "mesh_.owner()[iface]: "   << mesh_.owner()[iface] << endl;
                    Info << "mesh_.Sf()[iface]: "   << mesh_.Sf()[iface] << endl;
                    Info << "faces[iface]: "   << mesh_.faces()[iface] << endl;
                    Info << "edgenumberfacei: " << edgenumberfacei << endl;
                    Info << "edgenumberfacei[edgei]: " << edgenumberfacei[edgei] << endl;
                    Info << "edgenumberfacei[edgei]: " << mesh_.edges()[edgenumberfacei[edgei]] << endl;
                    Info << "startPoint : " << startPoint << endl;
                    Info << "endPoint : "   << endPoint << endl;
                    Info << "mesh_.points()[startPoint]: "   << mesh_.points()[startPoint] << endl;
                    Info << "mesh_.points()[endPoint]: "     << mesh_.points()[endPoint] << endl;
                    Info << "x2: " << x2 << endl;
                    cin.get();
