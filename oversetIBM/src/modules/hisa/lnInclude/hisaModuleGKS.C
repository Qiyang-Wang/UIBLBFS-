...
namespace Foam
{
surfaceVectorField hisaModule::viscousFlux(const volScalarField& muEff)
{
    ...
    (
        ...
        mesh,
        dimensionedVector("Fv", dimPressure*dimArea, vector(0.0, 0.0, 0.0))
    );

    surfaceTensorField gradUs = linearInterpolate(fvc::grad(U));
//    gradUs.write();
//    (fvc::grad(U))().write();
    surfaceScalarField muEffs = linearInterpolate(muEff);
    
    forAll( mesh.Sf(), i )
    {
        Fv[i] = centralDiff(gradUs[i],Sf[i],muEffs[i]);
    }

    forAll(mesh.boundary(), patchi)
    {
        const vectorField& pSf = Sf.boundaryField()[patchi];
        const tensorField& pGradUs = gradUs.boundaryField()[patchi];
        const scalarField& pMuEffs = muEffs.boundaryField()[patchi];
        fvsPatchField<Vector<double>>& pFv = Fv.boundaryFieldRef()[patchi];
  
        forAll(mesh.boundary()[patchi], facei)
        {
            pFv[facei]=centralDiff(pGradUs[facei], pSf[facei], pMuEffs[facei]);
        }
    } 
...
vector hisaModule::centralDiff(tensor gradU, vector Sn, scalar muEff)
{
    vector S;

   // Strain rate and divergence of velocity field
    S.x() = gradU.xx()*Sn.x() + gradU.xy()*Sn.y() + gradU.xz()*Sn.z() +
            gradU.xx()*Sn.x() + gradU.yx()*Sn.y() + gradU.zx()*Sn.z();
    S.y() = gradU.yx()*Sn.x() + gradU.yy()*Sn.y() + gradU.yz()*Sn.z() +
            gradU.xy()*Sn.x() + gradU.yy()*Sn.y() + gradU.zy()*Sn.z();
    S.z() = gradU.zx()*Sn.x() + gradU.zy()*Sn.y() + gradU.zz()*Sn.z() +
            gradU.xz()*Sn.x() + gradU.yz()*Sn.y() + gradU.zz()*Sn.z();

    return -muEff*S;
}


surfaceTensorField hisaModule::gradInterpolate
(
    volTensorField grad
)
{
    return linearInterpolate(grad);
}

surfaceTensorField hisaModule::calcStress
(
    surfaceTensorField sf_old
)
{
    const Field<tensor>& sfi_old = sf_old;

    const fvMesh& mesh = sf_old.mesh();

    tmp<GeometricField<tensor, fvsPatchField, surfaceMesh>> tsf
    (
        new GeometricField<tensor, fvsPatchField, surfaceMesh>
        (
            IOobject
            (
                "stress("+sf_old.name()+')',
    ...
    GeometricField<tensor, fvsPatchField, surfaceMesh>& sf = tsf.ref();

    Field<tensor>& sfi = sf.primitiveFieldRef();

    for (label fi=0; fi<sfi_old.size(); fi++)
    {
        sfi[fi].xx() = 4/3*sfi_old[fi].xx() - 2/3*(sfi_old[fi].yy()+sfi_old[fi].zz());
        sfi[fi].yy() = 4/3*sfi_old[fi].yy() - 2/3*(sfi_old[fi].xx()+sfi_old[fi].zz());
        sfi[fi].zz() = 4/3*sfi_old[fi].zz() - 2/3*(sfi_old[fi].xx()+sfi_old[fi].yy());
        sfi[fi].xy() = sfi_old[fi].yx() + sfi_old[fi].xy();
        sfi[fi].yx() = sfi[fi].xy();
        sfi[fi].xz() = sfi_old[fi].zx() + sfi_old[fi].xz();
        sfi[fi].zx() = sfi[fi].xz();
        sfi[fi].yz() = sfi_old[fi].zy() + sfi_old[fi].yz();
        sfi[fi].zy() = sfi[fi].yz();
    }
  
    typename GeometricField<tensor, fvsPatchField, surfaceMesh>::
        Boundary& sfbf = sf.boundaryFieldRef(); 
    typename GeometricField<tensor, fvsPatchField, surfaceMesh>::
        Boundary& sfbf_old = sf_old.boundaryFieldRef(); 

    forAll(sf_old.boundaryField(), pi)
    {
        forAll(sfbf[pi],fi)
        {
	    sfbf[pi][fi].xx() = 4/3*sfbf_old[pi][fi].xx() - 2/3*(sfbf_old[pi][fi].yy()+sfbf_old[pi][fi].zz());
	    sfbf[pi][fi].yy() = 4/3*sfbf_old[pi][fi].yy() - 2/3*(sfbf_old[pi][fi].xx()+sfbf_old[pi][fi].zz());
	    sfbf[pi][fi].zz() = 4/3*sfbf_old[pi][fi].zz() - 2/3*(sfbf_old[pi][fi].xx()+sfbf_old[pi][fi].yy());
	    sfbf[pi][fi].xy() = sfbf_old[pi][fi].yx() + sfbf_old[pi][fi].xy();
	    sfbf[pi][fi].yx() = sfbf[pi][fi].xy();
	    sfbf[pi][fi].xz() = sfbf_old[pi][fi].zx() + sfbf_old[pi][fi].xz();
	    sfbf[pi][fi].zx() = sfbf[pi][fi].xz();
	    sfbf[pi][fi].yz() = sfbf_old[pi][fi].zy() + sfbf_old[pi][fi].yz();
	    sfbf[pi][fi].zy() = sfbf[pi][fi].yz();
        }
    }
...
void hisaModule::outerIteration()
{
    ...
    #include "ptrRefs.H"
    if (solnControl.firstIter() || steadyState_ || moveMeshOuterCorrectors_)
    {
    ...
    } else if(fluxType=="LBFS") {
        // Update residuals
//        #include "residualsUpdateVis.H"
        #include "residualsUpdateLBFS.H"
    } else {
        // Update residuals
//        #include "residualsUpdateOrg.H"
        #include "residualsUpdateCentral.H"
    }

    Info << runTime.deltaT() << endl;
    rhoUR.ref() = rhoUR.ref() + UTrans_()/mesh.V()/runTime.deltaT();
//    Info << rhoUR.dimensions() << endl;
//    cin.get();
    ...
    // Solver
    {
        ...
        scalar rhoRes  = gSum(mag(scalarResiduals_[0].primitiveField()));
        scalar rhoURes = gSum(mag(vectorResiduals_[0].primitiveField()));
     
        Info <<"*** rho  residual sum: " << rhoRes  << endl;
        Info <<"*** rhoU residual sum: " << rhoURes  << endl;

        solnControl_->setResidual(initRes_());
        solnControl_->setRhoResidual(rhoRes);
        solnControl_->setRhoUResidual(rhoURes);
    }
    ...
    if((fluxType=="GKS3D")||(fluxType=="GKS3DUnstruct")) {
        ...
        // Update fields
        #include "updateFieldsOrg.H"
    } else {
        // Update fields
        #include "updateFieldsOrg.H"
//        #include "updateFieldsIncomp.H"
    }
...
void hisaModule::turbulenceCorrect()
{
    // Correct turbulence
    turbulence_->correct();
}
